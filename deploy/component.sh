...
export TFS_REGISTRY_IMAGES=${TFS_REGISTRY_IMAGES:-"localhost:32000/tfs/"}
...
for COMPONENT in $TFS_COMPONENTS; do
    ...
    if [ -n "$TFS_REGISTRY_IMAGES" ]; then
        ...
        if [ "$COMPONENT" == "pathcomp" ]; then
            ...
            docker tag "$COMPONENT-connector:$TFS_IMAGE_TAG" "$IMAGE_URL-connector" > "$TAG_LOG"

            TAG_LOG="$TMP_LOGS_FOLDER/tag_${COMPONENT}-gateway.log"
            docker tag "$COMPONENT-gateway:$TFS_IMAGE_TAG" "$IMAGE_URL-gateway" > "$TAG_LOG"

            PUSH_LOG="$TMP_LOGS_FOLDER/push_${COMPONENT}-connector.log"
            docker push "$IMAGE_URL-connector" > "$PUSH_LOG"

            PUSH_LOG="$TMP_LOGS_FOLDER/push_${COMPONENT}-gateway.log"
            docker push "$IMAGE_URL-gateway" > "$PUSH_LOG"
        else
    ...
    if [ -n "$TFS_REGISTRY_IMAGES" ]; then
        # Registry is set
        if [ "$COMPONENT" == "pathcomp" ]; then
            sed -i "s|image: .*pathcomp-frontend.*|image: $IMAGE_URL-frontend|g" "$MANIFEST"
            sed -i "s|image: .*pathcomp-backend.*|image: $IMAGE_URL-backend|g" "$MANIFEST"
            sed -i "s|imagePullPolicy: .*|imagePullPolicy: Always|g" "$MANIFEST"
        elif [ "$COMPONENT" == "dlt" ]; then
            sed -i "s|image: .*dlt-connector.*|image: $IMAGE_URL-connector|g" "$MANIFEST"
            sed -i "s|image: .*dlt-gateway.*|image: $IMAGE_URL-gateway|g" "$MANIFEST"
            sed -i "s|imagePullPolicy: .*|imagePullPolicy: Always|g" "$MANIFEST"
        else
            sed -i "s|image: .*|image: $IMAGE_URL|g" "$MANIFEST"
            sed -i "s|imagePullPolicy: .*|imagePullPolicy: Always|g" "$MANIFEST"
        fi
    else
        # Registry is not set
        if [ "$COMPONENT" == "pathcomp" ]; then
            sed -i "s|image: .*pathcomp-frontend.*|image: $IMAGE_NAME-frontend|g" "$MANIFEST"
            sed -i "s|image: .*pathcomp-backend.*|image: $IMAGE_NAME-backend|g" "$MANIFEST"
            sed -i "s|imagePullPolicy: .*|imagePullPolicy: Never|g" "$MANIFEST"
        else
            sed -i "s|image: .*|image: $IMAGE_NAME|g" "$MANIFEST"
            sed -i "s|imagePullPolicy: .*|imagePullPolicy: Never|g" "$MANIFEST"
        fi
    fi
