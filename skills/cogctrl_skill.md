# CogCtrl Succession Skill (Portable Agent Instructions)

This skill mandatory for any AI or human interacting with the CogCtrl project. It ensures that the millions of failures encountered in previous iterations are never repeated.

## 1. Primary Directives

### Mandatory Documentation-First Workflow
- **Rule**: If a bug is encountered or a dependency is missing, you MUST first update this skill file or the `cogctrl_spec.md` before applying the fix.
...
- **Rule**: Enforce a 2-minute cooldown between service starts to prevent gRPC handshake timeouts under heavy CPU contention.
...
3.  **13-Screen Audit**: All 13 WebUI modules load correctly (see `cogctrl_spec.md`).
